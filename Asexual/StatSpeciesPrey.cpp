...
void StatSpeciesPrey::write_Header(string Prey, string PreyFCM, int gen) {
	...
	if ((gen == 1) || ((gen % 1000) == 0)) {
		...
		statFile << ", concept_escape, concept_searchFood, concept_searchFood2, concept_socialize, concept_exploration, concept_wait, concept_eat, concept_eat2, concept_reproduceAsexual";	//RS
#else
		...
		statFile << ", concept_fear, concept_hunger, concept_searchPartner, concept_curiosity, concept_sedentary, concept_satisfaction, concept_nuisance";
		statFile << ", concept_escape, concept_searchFood, concept_socialize, concept_exploration, concept_wait, concept_eat, concept_reproduceAsexual, concept_move2StrongestPrey, concept_move2StrongestPreyCell, concept_move2WeakestPreyCell\n";	//RS
#endif
...
void StatSpeciesPrey::reset() {
	...
	//reproduceFailedAsexual = 0;			//Number of reproduce asexual fails
	//reproduceFailedAsexualRatio = 0;	//Ratio of reproduce asexual fails
...
void StatSpeciesPrey::write(ofstream *statFile) {
	...
	*statFile << ", " << Manip.vtos((float)nbBirth / (float)nbSameGenderInds) << ", " << Manip.vtos(0); //RS
	...
	*statFile << ", " << (actions[FCMPrey::ReproduceAsexualFailAction - 1] > 0 ? Manip.vtos((float)actions[FCMPrey::ReproduceAsexualFailAction - 1]) : Manip.vtos(0)) << ", " << (actions[FCMPrey::ReproduceAsexualFailAction - 1] > 0 ? Manip.vtos(((float) actions[FCMPrey::ReproduceAsexualFailAction - 1]) / ((float)actions[FCMPrey::ReproduceAsexualAction - 1] + (float)actions[FCMPrey::ReproduceAsexualFailAction - 1])) : Manip.vtos(0));
	...
	//*statFile << ", " << (actions[FCMPrey::ReproduceFailAction - 1] > 0 ? Manip.vtos((float)reasonReproduceFailed_Energy / (float)(actions[FCMPrey::ReproduceFailAction - 1])) : Manip.vtos(0)) << ", " << (actions[FCMPrey::ReproduceFailAction - 1] > 0 ? Manip.vtos((float)reasonReproduceFailed_NoPartner / (float)(actions[FCMPrey::ReproduceFailAction - 1])) : Manip.vtos(0));
	//*statFile << ", " << (actions[FCMPrey::ReproduceFailAction - 1] > 0 ? Manip.vtos((float)reasonReproduceFailed_PartnerEnergy / (float)(actions[FCMPrey::ReproduceFailAction - 1])) : Manip.vtos(0)) << ", " << (actions[FCMPrey::ReproduceFailAction - 1] > 0 ? Manip.vtos((float)reasonReproduceFailed_PartnerActed / (float)(actions[FCMPrey::ReproduceFailAction - 1])) : Manip.vtos(0));
	//*statFile << ", " << (actions[FCMPrey::ReproduceFailAction - 1] > 0 ? Manip.vtos((float)reasonReproduceFailed_PartnerAction / (float)(actions[FCMPrey::ReproduceFailAction - 1])) : Manip.vtos(0)) << ", " << (actions[FCMPrey::ReproduceFailAction - 1] > 0 ? Manip.vtos((float)reasonReproduceFailed_PartnerDist / (float)(actions[FCMPrey::ReproduceFailAction - 1])) : Manip.vtos(0));
	//*statFile << ", " << (actions[FCMPrey::ReproduceFailAction-1] > 0 ? Manip.vtos((float)reasonReproduceFailed_SameGender / (float)(actions[FCMPrey::ReproduceFailAction-1])) : Manip.vtos(0)) << ", " << (actions[FCMPrey::ReproduceFailAction-1] > 0 ? Manip.vtos((float) reasonReproduceFailed_SameGender / (float)(actions[FCMPrey::ReproduceFailAction-1])) : Manip.vtos(0));
...
void StatSpeciesPrey::writeSpeciesStat(Ecosystem * eco, int gender) {
	...
	for (list<Species>::iterator it = eco->speciesPrey.begin(); it != eco->speciesPrey.end(); ++it) {
		...
		for (int i = 0; i < nbIndividuals; i++){
			...
			if ((*it).preyMembers.at(i)->getLastAction() == FCMPrey::ReproduceAsexualAction){
				DistMating = 0; //RS
			}
			...
			/*if ((*it).preyMembers.at(i)->getLastAction() == FCMPrey::ReproduceFailAction){
				...
				//reasonReproduceFailed_SameGender		+= (*it).predMembers.at(i)->reproduceFailed6; //MRE
			}*/
		}
