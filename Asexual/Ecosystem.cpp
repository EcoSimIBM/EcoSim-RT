...
void Ecosystem::updateEco() {
	...
	for (unsigned int i = 0; i < rabbits.size(); i++) { // Act based on strength M.M; who has more strength act first
		...
		//if (rabbits[i].getGender() == 1)//1 means Gender::Female
			//if (rabbits[i].get_action_offset() == FCMPrey::Reproduce)
				//femaleRabbits.push_back((Prey) rabbits[i]); RS -> refer to ori if this breaks
	...
	*/
	...
	//here RS
	printf("\nNumber of preds before: %d", wolves.size());
	vector<Predator> newArtificialPredators;
	IncreasePredatorPressure(&newArtificialPredators);
	for (size_t i = 0; i < newArtificialPredators.size(); ++i) {
		wolves.push_back(newArtificialPredators[i]);
	}
	printf("\nNumber of preds after: %d", wolves.size());
	//RS end
...
void Ecosystem::minSave() {
	...
	for (i = 0; i < rabbits.size(); i++) {
		...
		if (rabbits[i].getID() <= PrevRabbitsLastID) {
			...
			/*if (rabbits[i].getLastAction() == FCMPrey::ReproduceFailAction) //-- Meisam: Reproduction failed  RS
			{
...
void Ecosystem::IncreasePredatorPressure(vector<Predator> * newPreds){
	if (generation % 20 == 0 && generation > 15000){	//Every 20 time steps
		...
		int timestepsOver = generation - 15000;
		float percentIncrease = (float)timestepsOver / 25.0;
		percentIncrease = percentIncrease / 20000; //modifier to determine rate of increase if necessary
		int numberIndividualsToAdd = (int)(rabbits.size() * percentIncrease); //increase predator population artificially by percentIncrease
		for (int k = 0; k < numberIndividualsToAdd; k++){
			
			//select random pred to duplicate
			int randIndex = rand.next(0, wolves.size() - 1);
			
			//duplicate it
			//use the code from reprod
			short bebe_gender = wolves[randIndex].getGender(); //Add Gender /MRE/ Female=1, Male=0
			//-- Create the new Pred
			short bebe_type = 1; //--Pred
			long bebe_id = maxPredID;
			
			//need random position for baby
			Position bebe_position;
			bebe_position.x = rand.next(width);
			bebe_position.y = rand.next(height);
			
			FCMPredator bebe_fcm = FCMPredator(NULL);
			Predator *selected_parent = &(wolves[randIndex]);
			...
			for (int i = 0; i < nSens + nConcepts + nMoteursDep + nMoteursFix; ++i) { //-- Generate the new Proie's concepts and chart
				for (int j = 0; j < nConcepts + nMoteursDep + nMoteursFix; ++j) {
					bebe_fcm.set_chart(i, j, selected_parent->model.get_chart(i, j));
				}
			}
			...
			//use the new baby constructor
			Predator * bebe = new Predator(bebe_type, bebe_id, wolves[randIndex].getSpecies(), wolves[randIndex].getParent1ID(), wolves[randIndex].getParent2ID(), wolves[randIndex].getSpeciesParent1ID(), wolves[randIndex].getSpeciesParent2ID(), bebe_position, 0, wolves[randIndex].getAge(), wolves[randIndex].getDistanceEvol(), &bebe_fcm, bebe_gender); //Add Gender /MRE/

			//set the physical genome
			(bebe->getPhGenome())->crossover((wolves[randIndex].getPhGenome())->getGenome(), (wolves[randIndex].getPhGenome())->getGenome(), bebe_gender, (wolves[randIndex].getPhGenome())->persuasion); //maybe?
			
			//set the energy = wolves[randIndex].getEnergy()
			bebe->setEnergy(wolves[randIndex].getEnergy());
			
			bebe->speciesPtr = wolves[randIndex].speciesPtr;
			
			//add it back to predators list
			bebe->set_action_offset(-1);
			newPreds->push_back(*bebe);	//-- Add the bebe to newPred
...
void Ecosystem::TwoMeanPreySpeciation() {
	...
	*/
	...
	for (list<Species>::iterator S = speciesPrey.begin(); S != speciesPrey.end(); ++S) {
		...
		if (fDistance > 1.4 * distanceSpeciesPrey) {
