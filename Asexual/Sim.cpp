...
float Sim::calculationEnergyAction(float energy, int nbArc, float speed, float strength, float persuasion, Ecosystem * eco){
	...
	if (phGenome.getGenomeSize() > 6){ // prey
		...
		float energyToSpend = 0.9 * ((.8 * arcPenalty)  + ((strength*(pow((double)speed, 2))) / 10000.0) + pow((double)maxEnergy / 5.5, 0.75) + pow((double)vision * 5.0, 0.75) + (pow((double)maxSpeed * 5.0, 0.75)) + pow((double)defence * 100.0, 0.75) + pow((double)coopDefence * 75.0, 0.75) + pow((double)max(0.0, (double)(8 - repAge)), 2.3) + pow((double)persuasion * 5.0, 0.75));
		...
		float energyToSpend = 0.9 * ((.8 * arcPenalty)  + ((strength*(pow((double)speed, 2))) / 11000.0) + pow((double)maxEnergy / 5.5, 0.75) + pow((double)vision * 5.0, 0.75) + pow((double)maxSpeed * 5.0, 0.75) + pow((double)max(0.0, (double)(7 - repAge)), 2.3) + pow((double)persuasion * 5.0, 0.75));
		e = energy - energyToSpend;
		if (getGender() == 0){
...
//RS OLD
/*
float Sim::calculationEnergyNewBornAsexual(float sobParent1, float EnergyMaxParent1, int minimumSob){
...
//RS OLD
/*
float Sim::calculationEnergyNewParentAsexual(float energyParent1, float energyNewBorn, float sobParent1){
	//Parent1 should have the same gender with bebe
	//1.05: waste energy for reproduction
	float energy = energyParent1 - (energyNewBorn * 1.05);
	//cout << energy << endl;
	return energy;
}*/
...
float Sim::calculationEnergyNewBornAsexual(float sobParent1, float EnergyMaxParent1){

	float energy = (EnergyMaxParent1 * sobParent1 * 1.9) / (float)100;
	return energy;
}
...
float Sim::calculationEnergyNewParentAsexual(float energyParent1, float energyNewBorn, float sobParent1){
	//Parent1 should have the same gender with bebe
	//1.05: waste energy for reproduction
	float energy = energyParent1 - (energyNewBorn * 1.05) * 1.3;
	//cout << energy << endl;
	return energy;
}
