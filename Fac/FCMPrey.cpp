...
int FCMPrey::choose_action(Ecosystem *eco){
	...
	for (int i = nbSens + nbConcepts; i < nbSens + nbConcepts+nbMoteursDep + nbMoteursFix; i++) {				//-- nbMoteursDep=4, fix=3
		...
		if (change_action == true) {
			if (i == Reproduce) {	// RS				//-- Change it Armin
			...
			}else if (i == ReproduceAsexual) {	// RS				//-- Change it Armin
				if (prey->getAge() >= prey->getRepAge() && prey->getGender() == 1){ // M.M because Rep Age now is opart of genome.
					max = activations[i];
					numMax = (short)i;
				}
			} else {
